...
const LetterSwapForward = ({
	...
	return (
		<span className={`flex justify-start items-center relative overflow-hidden ${className}`} onMouseEnter={hoverStart} onClick={onClick} ref={scope} {...props}>
			<span className='sr-only'>{label}</span>

			{label.split('').map((letter: string, i: number) => {
				return (
					<span className='relative flex whitespace-pre' key={i}>
						<motion.span className={`letter relative`} style={{ top: 0 }}>
							{letter}
						</motion.span>
						<motion.span className='letter-secondary absolute' aria-hidden={true} style={{ top: reverse ? '-100%' : '100%' }}>
							{letter}
						</motion.span>
					</span>
				);
